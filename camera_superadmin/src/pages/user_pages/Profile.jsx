...
// no flickering and faster page, using useCallBack , and useMemo
...
export default function Profile() {
  ...
  useEffect(() => {
    preloadFallbackImage(); // avoid flickering
    const fetchUserData = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.get(
  ...
  const handleUpdateProfile = useCallback(() => {
    if (userData?._id) navigate(`/update-profile/${userData._id}`);
  }, [navigate, userData]);

  const getImageUrl = useCallback(() => {
    if (!userData?.avatar || imageError) return fallbackImage;
    return `${globalBackendRoute}/${userData.avatar.replace(/\\/g, "/")}`;
  }, [userData, imageError]);
